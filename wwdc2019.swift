...
class wwdcVideosController {
    ...
    class func getSessionsList(fromHTML: String) -> Array<String> {
        let pat = "\"\\/videos\\/play\\/wwdc2019\\/([0-9]*)\\/\""
        let regex = try! NSRegularExpression(pattern: pat, options: [])
        let matches = regex.matches(in: fromHTML, options: [], range: NSRange(location: 0, length: fromHTML.count))
...
func showHelpAndExit() {
    print("wwdc2019 - a simple swifty video sessions bulk download.\nJust Get'em all!")
    print("usage: wwdc2019.swift [--hd] [--sd] [--pdf] [--pdf-only] [--sessions] [--sample] [--list-only] [--help]\n")
    exit(0)
}
...
var skipSessionsSet:Set<String> = Set()
...
for argument in arguments {
    ...
    case _ where Int(argument) != nil:
        if(!gettingSessions) {
            print("not getting session! --sessions before you may specify session id")
            fallthrough
        }
        if(argument.starts(with:"-")) {
            let s = String(argument.dropFirst(1))
            skipSessionsSet.insert(s)
            print("skip \(s)")
        } else {
...
let htmlSessionListString = wwdcVideosController.getStringContent(fromURL: "https://developer.apple.com/videos/wwdc2019/")
...
if skipSessionsSet.count != 0 {
    let sessionsListSet = Set(sessionsListArray)
    sessionsListArray = Array(sessionsListSet.subtracting(skipSessionsSet))
    print("skip sessions applied, \(sessionsListArray.count) left")
}

sessionsListArray.sort(by: sortFunc)

for (_, value) in sessionsListArray.enumerated() {
    let htmlText = wwdcVideosController.getStringContent(fromURL: "https://developer.apple.com/videos/play/wwdc2019/" + value + "/")
